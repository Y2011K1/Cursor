        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {list.length > 0 ? (
                list.map((lesson: any, index: number) => {
                  const isCompleted = progressMap.get(lesson.id) ?? false
                  return (
                    <Card key={lesson.id} className="border-none shadow-md hover:shadow-lg transition-shadow rounded-2xl">
                      <CardHeader>
                      ...
                      <CardContent className="space-y-3">
                        {lesson.content && <p className="text-sm text-slate-blue line-clamp-2">{lesson.content}</p>}
                        <div className="flex items-center justify-between">
                          ...
                          <Button size="sm" asChild>
                            <Link href={`/dashboard/student/course/${courseId}/lesson/${lesson.id}`}>View Lesson</Link>
                          </Button>
                        </div>
                      </CardContent>
                    </Card>
