        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {list.length > 0 ? (
                list.map((quiz: any) => (
                  <Card key={quiz.id} className="border-none shadow-md hover:shadow-lg transition-shadow rounded-2xl">
                    <CardHeader>
                      <CardTitle className="text-deep-teal text-base">{quiz.title}</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-3">
                      {quiz.description && <p className="text-sm text-slate-blue line-clamp-2">{quiz.description}</p>}
                      ...
                      <Button size="sm" className="w-full" asChild>
                        <Link href={`/dashboard/student/course/${courseId}/quiz/${quiz.id}`}>Take Quiz</Link>
                      </Button>
                    </CardContent>
                  </Card>
                ))
                ...
                <Card className="border-none shadow-sm rounded-2xl bg-white/80">
                  <CardContent className="py-6 text-center text-slate-blue text-sm">
                    No assignments in this course yet. Check back later.
                  </CardContent>
                </Card>
              )}
            </div>
