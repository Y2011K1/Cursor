...
interface ProfilePictureUploadProps {
  ...
  disabled?: boolean
  className?: string
  /** Square preview (how it appears on landing page) when true; otherwise allow rectangle. */
  square?: boolean
  /** Called when a new URL is set (after upload or clear). Use to sync avatar preview. */
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      {value ? (
        <div
          className={cn(
            "relative rounded-xl border-2 border-deep-teal/20 overflow-hidden bg-muted/30",
            aspectClass,
            "w-full max-w-[240px]"
          )}
        >
        ...
        <div
          ...
          onClick={() => inputRef.current?.click()}
          className={cn(
            "border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-2 p-6 min-h-[180px] w-full max-w-[240px] cursor-pointer transition-colors",
            aspectClass,
        ...
        >
          ...
          {uploading ? (
            ...
            <>
              ...
              <span className="text-sm font-medium text-deep-teal text-center">
                Drop a photo here or click to upload
              </span>
              <span className="text-xs text-slate-500 text-center">
                Square or rectangle, max 5MB. Fits on landing page.
              </span>
            </>
          )}
        </div>
